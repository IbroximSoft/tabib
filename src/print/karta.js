/* ============================================================
   KASALLIK VARAQASI — BITTA A4 VARAQ, YOTIQ (landshaft), 2 TOMON

     Varaq o'rtasidan ikkiga bo'linadi. Har yarmi — kartaning
     bitta beti (A5 o'lchamida). Shunday qilib:

       OLD tomoni :  [ 1-bet: bemor ma'lumotlari ] | [ 2-bet: I–II kurs ]
       ORQA tomoni:  [ 3-bet: III–IV kurs        ] | [ 4-bet: yakun + TILXAT ]

     Ya'ni butun karta — 1 dona A4 qog'oz, ikki tomonlama.
     Chiqargandan keyin o'rtasidagi punktir chiziq bo'yicha
     buklanadi yoki qirqiladi.

   OLDI va ORQASI ALOHIDA CHIQADI
     kartaHtml(y, 'old')   → faqat 1 varaq (1- va 2-bet)
     kartaHtml(y, 'orqa')  → faqat 1 varaq (3- va 4-bet)
     kartaHtml(y)          → ikkalasi ketma-ket (2 varaq)
     Printerda ikki tomonlama rejim bo'lmasa: avval oldini
     chiqarib, qog'ozni ag'darib, keyin orqasini chiqariladi.

   Bazada yo'q maydonlar (otasining ismi, manzil) qo'lda
   to'ldirish uchun chiziq bo'lib chiqadi — qog'ozdagidek.
   ============================================================ */
import { esc, kun } from '../lib/chop'
import { MARKAZ } from './chek'

/* Tabib ismi tilxat matnida keladi */
export const TABIB = 'Abdurahimov Muhiddin Abduvohobovich'

/* Markaz logotipi — chop etish uchun fayl ichiga joylangan.
   Tashqi manzilga bog'lanmaydi, shuning uchun internetsiz ham,
   yashirin chop ramkasida ham albatta chiqadi. */
export const LOGO = 
  'data:image/png;base64,'

/* Chop etish tomonlari — ekrandagi tugmalar shu ro'yxatdan chiziladi */
export const TOMONLAR = [
  { k: 'old', n: 'Old tomoni', tavsif: '1-bet: maʼlumotlar · 2-bet: I–II kurs' },
  { k: 'orqa', n: 'Orqa tomoni', tavsif: '3-bet: III–IV kurs · 4-bet: yakun va tilxat' }
]

export const kartaCss = `
@page { size: A4 landscape; margin: 6mm; }
* { box-sizing: border-box; }
body { margin: 0; font: 10px/1.35 "Times New Roman", Georgia, serif; color: #000 }

/* --- bitta A4 varaqning bir tomoni --- */
.varaq { display: flex; width: 285mm; height: 198mm;
         page-break-after: always; page-break-inside: avoid; overflow: hidden }
.varaq:last-child { page-break-after: auto }

/* --- varaqning yarmi = kartaning bitta beti --- */
.yarim { width: 142.5mm; height: 198mm; padding: 0 5mm; overflow: hidden;
         display: flex; flex-direction: column }
.yarim.ikki { border-left: 1px dashed #9a9a9a }
.tana { flex: 1; min-height: 0 }
.oyoq { flex: none; margin-top: 2mm; padding-top: 1.4mm; border-top: 1px solid #000;
        font-size: 7.5px; color: #333; display: flex; justify-content: space-between }

/* --- sarlavha --- */
.shapka { display: flex; align-items: flex-start; gap: 3mm; margin-bottom: 3mm }
.gerb { width: 15mm; height: auto; flex: none; object-fit: contain }
.shapka-m { flex: 1; text-align: center; font-size: 8.5px; line-height: 1.3;
            text-transform: uppercase; letter-spacing: .01em }
.shapka-m b { font-size: 9.5px }
.nomer { flex: none; display: flex; align-items: center; gap: 1mm; font-size: 9px }
.nomer i { width: 4mm; height: 5mm; border: 1px solid #000; display: block; font-style: normal }

h1 { font-size: 10.5px; text-align: center; text-transform: uppercase;
     margin: 2mm 0 3mm; line-height: 1.3; font-weight: 700 }
h2 { font-size: 10px; margin: 3mm 0 1.5mm; font-weight: 700 }

/* --- to'ldiriladigan maydonlar --- */
.maydon { display: flex; align-items: baseline; gap: 2mm; margin-bottom: 6.8mm }
.maydon > label { flex: none; font-size: 9px; white-space: nowrap }
.qiymat { flex: 1; border-bottom: 1px solid #000; min-height: 6.2mm; padding: 0 1mm .4mm;
          font-size: 10.5px; font-weight: 600 }
.qiymat.bosh { font-weight: 400 }
.katak { display: flex; gap: 0.8mm }
.katak i { width: 4mm; height: 5mm; border: 1px solid #000; display: block; font-style: normal }
.chiziq { border-bottom: 1px solid #000; height: 8mm; margin-bottom: 1.6mm }

/* --- kurs jadvali --- */
.kurs { border: 1px solid #000; padding: 1.5mm 2mm 2mm; margin-bottom: 5mm }
.kurs:last-child { margin-bottom: 0 }
.kurs h3 { font-size: 10px; text-align: center; margin: 0 0 1mm; font-weight: 700 }
.kurs .sana { text-align: center; font-size: 9px; margin-bottom: 1.5mm }
.kurs .sana u { display: inline-block; min-width: 20mm; border-bottom: 1px solid #000;
                text-decoration: none }
.qator { display: flex; align-items: center; gap: 1.5mm; height: 10mm }
.qator .k { flex: none; width: 13mm; font-size: 9px }
.qator .sn { flex: none; width: 18mm; height: 6.4mm; border: 1px solid #000 }
.qator .ml { flex: 1; border-bottom: 1px solid #000; height: 6.4mm; min-width: 0 }
.qator .im { flex: none; width: 22mm; height: 6.4mm; border: 1px solid #000 }
.qator-sar { display: flex; gap: 1.5mm; font-size: 7.5px; color: #444; margin-bottom: .6mm }
.qator-sar .k { flex: none; width: 13mm }
.qator-sar .sn { flex: none; width: 18mm; text-align: center }
.qator-sar .ml { flex: 1 }
.qator-sar .im { flex: none; width: 22mm; text-align: center }

/* --- tilxat --- */
.tilxat { border: 1px solid #000; padding: 2.5mm 3mm 3mm }
.tilxat h2 { text-align: center; margin: 0 0 2mm; letter-spacing: .08em; font-size: 10.5px }
.tilxat p { margin: 0 0 2mm; text-align: justify; font-size: 9.5px; line-height: 1.5 }
.tilxat .izoh { font-size: 7.5px; color: #444; text-align: center; margin-top: -1mm }
.imzo { display: flex; gap: 6mm; margin-top: 8mm }
.imzo > div { flex: 1 }
.imzo .chiz { border-bottom: 1px solid #000; height: 6mm }
.imzo .lab { font-size: 9px; margin-top: 1mm }

/* Ekranda ko'rish uchun (modal ichidagi oldindan ko'rish).
   Chop etishda @page hoshiyasi ishlaydi, ekranda esa yo'q —
   shuning uchun bu yerda varaq to'liq A4 o'lchamida chiziladi. */
@media screen {
  body { background: #e8e8e8; padding: 6mm }
  .varaq { width: 297mm; height: 210mm; padding: 6mm; background: #fff;
           margin: 0 auto 6mm; box-shadow: 0 1px 5px rgba(0,0,0,.28) }
  .varaq:last-child { margin-bottom: 0 }
}
`

/* ------------------------------------------------------------
   YORDAMCHI QISMLAR
   ------------------------------------------------------------ */
function kataklar(n) {
  return '<span class="katak">' + '<i></i>'.repeat(n) + '</span>'
}

function chiziqlar(n) {
  return '<div class="chiziq"></div>'.repeat(n)
}

/* bitta kurs jadvali (7 kun) — endi butun bet kengligida */
function kursBlok(nomi) {
  const qatorlar = []
  for (let i = 1; i <= 7; i++) {
    qatorlar.push(
      `<div class="qator">
         <span class="k">${i}-kun</span>
         <span class="sn"></span>
         <span class="ml"></span>
         <span class="im"></span>
       </div>`)
  }
  return `
<div class="kurs">
  <h3>${esc(nomi)} — Kurs muolajasi</h3>
  <div class="sana">« <u></u> » <u></u> 20<u style="min-width:8mm"></u></div>
  <div class="qator-sar">
    <span class="k"></span><span class="sn">sana</span>
    <span class="ml">muolaja qiluvchi</span><span class="im">imzo</span>
  </div>
  ${qatorlar.join('')}
</div>`
}

/* har bir betning pastki chizig'i — qaysi bet ekani ko'rinib tursin */
function oyoq(raqam, matn) {
  return `<div class="oyoq"><span>${esc(MARKAZ.nomi)}</span>
          <span>${esc(matn)} · ${raqam}-bet</span></div>`
}

/* varaqning bir tomoni: chap yarmi + o'ng yarmi */
function varaq(chap, ong) {
  return `<div class="varaq">
  <div class="yarim">${chap}</div>
  <div class="yarim ikki">${ong}</div>
</div>`
}

/* ------------------------------------------------------------
   BETLAR
   ------------------------------------------------------------ */

/* 1-bet — sarlavha, bemor ma'lumotlari, tashxis */
function bet1(y) {
  const joy = y.xonada ? `${y.xona}-xona, ${y.koyka}-koyka` : ''
  return `
<div class="tana">
  <div class="shapka">
    <img class="gerb" src="${LOGO}" alt="">
    <div class="shapka-m">
      O‘zbekiston xalq tabobati assotsiatsiyasiga qarashli<br>
      <b>“${esc(MARKAZ.nomi)}”</b><br>
      xususiy tabobat markazi
    </div>
    <div class="nomer">№ ${kataklar(4)}</div>
  </div>

  <h1>Muolaja olgan bemorning ma’lumotlar varaqasi</h1>

  <div class="maydon"><label>Familiyasi</label>
    <span class="qiymat">${esc(y.familiya || '')}</span></div>
  <div class="maydon"><label>Ismi</label>
    <span class="qiymat">${esc(y.ism || '')}</span></div>
  <div class="maydon"><label>Otasining ismi</label>
    <span class="qiymat bosh"></span></div>
  <div class="maydon"><label>Telefon</label>
    <span class="qiymat">${esc(y.telefon || '')}</span></div>
  <div class="maydon"><label>Yoshi</label>
    <span class="qiymat">${y.yosh != null ? esc(y.yosh) + ' yosh' : ''}</span>
    <label>Jinsi</label>
    <span class="qiymat">${y.jins === 'ayol' ? 'Ayol' : 'Erkak'}</span></div>

  <div class="maydon"><label>Viloyati</label><span class="qiymat bosh"></span></div>
  <div class="maydon"><label>Shahri / tumani</label><span class="qiymat bosh"></span></div>
  <div class="maydon"><label>MFY, ko‘chasi</label><span class="qiymat bosh"></span></div>
  <div class="maydon"><label>Uy</label><span class="qiymat bosh"></span>
    <label>Xonadon</label><span class="qiymat bosh"></span></div>

  <div class="maydon"><label>Kelgan sanasi</label>
    <span class="qiymat">${esc(kun(y.kirish_sana))}</span>
    <label>Xona / koyka</label>
    <span class="qiymat">${esc(joy)}</span></div>

  <div class="maydon"><label>Kasallik tashxisi</label>
    <span class="qiymat">${esc(y.tashxis || '')}</span></div>
  ${chiziqlar(y.tashxis ? 1 : 2)}
</div>
${oyoq(1, 'Bemor maʼlumotlari')}`
}

/* 2-bet — I va II kurs */
function bet2() {
  return `
<div class="tana">
  <h2>Muolaja kunlari</h2>
  ${kursBlok('I')}
  ${kursBlok('II')}
</div>
${oyoq(2, 'I–II kurs')}`
}

/* 3-bet — III va IV kurs */
function bet3() {
  return `
<div class="tana">
  <h2>Muolaja kunlari (davomi)</h2>
  ${kursBlok('III')}
  ${kursBlok('IV')}
</div>
${oyoq(3, 'III–IV kurs')}`
}

/* 4-bet — muolaja yakuni va TILXAT */
function bet4(y) {
  return `
<div class="tana">
  <h2>Muolaja yakuni bo‘yicha bemorning holatidagi o‘zgarishlar</h2>
  ${chiziqlar(13)}

  <div class="tilxat">
    <h2>TILXAT</h2>
    <p>Men, <span style="display:inline-block;min-width:78mm;border-bottom:1px solid #000">
      &nbsp;${esc(y.fish || '')}&nbsp;</span></p>
    <div class="izoh">(bemorning ismi, sharifi, otasining ismi to‘liq yoziladi)</div>
    <p style="margin-top:2.5mm">
      o‘z xohishim va ixtiyorim bilan xalq tabobati usulida muolaja olish uchun tabib
      <b>${esc(TABIB)}ga</b> murojaat qildim. Menga og‘zaki kelishuv asosida muolaja
      shartlari va qoidalari to‘liq tushuntirildi. Muolaja jarayoniga men tomonimdan
      hech qanday e’tirozlarim yo‘q. Tilxatni o‘z qo‘llarim bilan to‘g‘ri yozdim.
    </p>

    <div class="imzo">
      <div><div class="chiz"></div><div class="lab">Sana</div></div>
      <div><div class="chiz"></div><div class="lab">Imzo</div></div>
    </div>
  </div>
</div>
${oyoq(4, 'Yakun va tilxat')}`
}

/* ------------------------------------------------------------
   TO'LIQ HTML
   tomon: 'old' | 'orqa' | undefined (ikkalasi)
   ------------------------------------------------------------ */
export function kartaHtml(y, tomon) {
  const old = varaq(bet1(y), bet2())
  const orqa = varaq(bet3(), bet4(y))
  if (tomon === 'old') return old
  if (tomon === 'orqa') return orqa
  return old + orqa
}
